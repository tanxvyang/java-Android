		...
		for (int i = 0; i < goods.size(); i++) {
			...
			buyButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					//获取购买数量
					String str = JOptionPane.showInputDialog(searchResultPane, "请输入要购买的数量", "购买数量", JOptionPane.QUESTION_MESSAGE);
					try {
						Integer buyNums = Integer.parseInt(str);
						gs.buy(user,good,buyNums);
					} catch (Exception e) {
						JOptionPane.showMessageDialog(searchResultPane, "输入的数量格式不正确", "错误提示", JOptionPane.ERROR_MESSAGE);
					}
				}
			});
